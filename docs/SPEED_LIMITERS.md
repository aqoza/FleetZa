...
| Field | Lives on | Why |
|---|---|---|
| `uin` | `sl_jobs` → `speed_limiter_installations` → `speed_limiter_certificates` | **Derived at issuance**: `OM-<last 5 digits of the vehicle's chassis>-<document number>` — chassis `JHHLCK1F7PK026626` on certificate `GOM-WO-202601` gives `OM-26626-202601`. `app.build_uin` owns the rule and `buildUin` in `src/lib/certificate.ts` mirrors it for the renewal path; the two must agree, because certificates are issued through both. Letters in the chassis are skipped (the rule counts digits) and a short chassis is left-padded to five. A vehicle with no chassis on file has nothing to derive from, so it falls back to the UIN recorded on the job or installation rather than printing a malformed identifier. **The UIN therefore changes with each renewal**, because the document number does; the columns on jobs and installations remain as the manual/legacy record and as that fallback. |
...
`translateIn(language, key)` exists in `src/i18n`. Dates on the document use
`formatDocumentDate` (DD-MM-YYYY, locale-independent) rather than `formatDate`.

Two deliberate departures from the scanned reference: section spacing is uniform
(the original carries a stray empty paragraph under SPEED LIMITER DETAILS), and
the "Generated by FleetManage / Verify at …" block is screen-only — the printed
document belongs to the dealer, and the QR already carries the verify URL.
