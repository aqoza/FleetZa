...
| `technician_name` | `sl_jobs` → `sl_technicians`, else `sl_settings.default_technician_id` | Snapshotted onto the certificate at issuance, like `limiter_type` and `tamper_seal_number`. Two reasons it is not read live through the job: a certificate issued **without** a job — imported history, or a document recorded after the fact — can never name a technician at all and prints a dash forever; and a live read is not a snapshot, so reassigning a job's technician would silently rewrite who is named on a document already handed to a customer. The print page and the verify endpoint both resolve `certificate.technician_name ?? job.technician.name`, so rows predating the column still work. Both issuance paths write it: `issue_certificate` (job → its technician, else the module default) and the renewal modal, which offers a dropdown of active technicians preselected from `sl_settings.default_technician_id` and falling back to whatever the certificate being replaced named. Bulk renewal has no per-row picker and uses that same resolution. |
...
| letterhead block | `tenants` | `name_ar`, `cr_number`, `po_box`, `postal_code`, `city`/`city_ar`, `email`, `website`, `phone_secondary`, `services_line`/`services_line_ar`, `signature_url`, `stamp_url`, `stamp_scale`, `applicable_standard`, `signatory_name`/`signatory_name_ar` — edited under Settings → Organization → Document letterhead. The two mark URLs are constrained to `https:` / `data:image` in the database and validated in the form. `stamp_scale` is the stamp's box height as a percent of the default (50-200, DB-constrained), offered in Settings as discrete steps: seals are scanned at different crops, so one fixed height renders some large and others tiny, and the bound is what keeps the closing strip on a single page. `applicable_standard` is the standard the fitted limiter is certified against (e.g. `GSO-1026/2002`), printed in the header row beside the country of installation — per tenant rather than per certificate because it applies to every vehicle a dealer certifies, and per tenant rather than hard-coded because GSO is the Gulf standard and this app is not GCC-only. `website` is deliberately unconstrained in the database — it is only ever printed as text, never dereferenced — but the form asks for the bare domain the dealer writes on the letterhead (`gawhrat.com`), since a pasted `https://…/path` would print in full. |
...
page chrome above the certificate (back link, copy-verify-link, download and
print buttons) is `print:hidden` and never reaches paper.
...
| Invoicing, quotes, payments for jobs/certificates | **Sales / Billing** modules (shipped — docs/SALES.md; certificates are billed per line, see *Billing certificates* there) |
